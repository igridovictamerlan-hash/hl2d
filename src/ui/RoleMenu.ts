...
export class RoleMenu {
  ...
  constructor(
    parent: HTMLElement,
    private readonly onChoose: (faction: FactionId, rank: number) => void,
  ) {
    ...
    this.el.querySelectorAll<HTMLButtonElement>('button[data-pick]').forEach((b) =>
      b.addEventListener('click', () => {
        ...
        this.close();
        this.onChoose(id, sel ? Number(sel.value) : 0);
      }),
    );
    // Смена ранга сразу красит кружок на карточке.
    this.el.querySelectorAll<HTMLSelectElement>('select').forEach((sel) =>
